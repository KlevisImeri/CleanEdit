...
const timelineWidth = computed(() => {
  if (!selectedProject.value?.video?.durationFPS) return 0;
  return selectedProject.value.video.durationFPS * fpsToPx.value;
});
const tracks = computed(() => selectedProject.value?.tracks || []);
...
const visibleSegments = computed(() => {
  ...
  if (!selectedProject.value?.tracks) return [];
...
const handleClick = (event: MouseEvent) => {
  ...
  const selectedSegment = binarySearch(track, newFrame);
